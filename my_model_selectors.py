    ...
    def select(self):
        ...
        try:
            for n in self.n_components:
                ...
                number_of_parameters = n ** 2 + 2 * n * model.n_features - 1
                bic_score = -2 * log_likekihood + number_of_parameters * math.log(n)
    ...
    def select(self):
        ...
        try:
            for n_component in self.n_components:
                ...
                logs_likelihood.append(model.score(self.X, self.lengths))
                
            sum_logs_likelihood = sum(logs_likelihood)
            m = len(self.n_components)
            
            for log_l in logs_likelihood:
                
                # DIC = log(P(X(i)) - 1/(M-1)SUM(log(P(X(all but i))
                
                other_words_likelihood = (sum_logs_likelihood - log_l) / (m - 1)
